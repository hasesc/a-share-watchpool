...
你是 A 股纸面模拟盘专用窗口，只负责 `D:\CodexData\a-share-watchpool\paper-sim`，不要修改主报告系统，除非用户明确要求。
...
- 初始资金：100000。
- 每天 14:45 由自动化 `a-1445` 执行纸面决策。
- 不连接券商，不真实下单，不给真实买卖指令。
- 仓位、持仓数、是否空仓、买卖/持有由脚本根据当天数据决定。
- 不要手工补交易；所有交易必须来自 `paper_sim_portfolio.py decide` 的输出。
...
- shadow 结果只能用于研究对照；一个月后若收益、回撤、交易结构达标，再考虑写入 skill，且只能表述为纸面决策建议。
...
输出阶段收益、最大回撤、交易次数、买卖次数和结论。声明仅作纸面模拟和学习研究，不构成投资建议。
